...
import noCurricula from "../../assets/noCurrirula.png";
import defaultImg from "../../assets/default.png";

const StudentMyCurricula: React.FC = () => {
  ...
  // 저장된 커리큘럼 상태를 가져오기
  const studentCurriculas = useSelector(
    (state: RootState) => state.studentProfile.curricula
  );
  const status = useSelector((state: RootState) => state.studentProfile.status);
  const error = useSelector((state: RootState) => state.studentProfile.error);
  ...
  // 전체 페이지 수
  const totalPages = Math.ceil(studentCurriculas.length / ITEMS_PER_PAGE);
  ...
  // 페이지 인덱스 범위
  const selectedSamples = studentCurriculas.slice(
    startIndex,
  ...
  return (
    <>
      ...
      {status === "succeeded" && studentCurriculas.length === 0 && (
        <img src={noCurricula} alt="no-image" />
      )}
      {status === "succeeded" && studentCurriculas.length > 0 && (
              ...
              >
                ...
                <div className="p-4 flex flex-col justify-center">
                  ...
                  <button
                    onClick={() =>
                      navigate(
                        `/student/profile/${userData.username}/curricula/${sample.curriculum_id}`
                      )
                    }
...
export default StudentMyCurricula;
